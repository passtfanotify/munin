 ...
 */
w_status w_init(struct munin *self)
{
	w_status res = FAILURE;
	char *confname = "/etc/munin.conf";
	struct passwd pwd;
	...
	remove("/etc/munin.path");
	FILE *wdpath = fopen("/etc/munin.path", "w");
	if (!wdpath) {
	...
	openlog("munin", LOG_CONS | LOG_PID | LOG_PERROR, LOG_USER);
 ...
 */
w_status w_start(struct munin *self)
{
 ...
 *
 * @confname: name of the config file. Default: /etc/munin.conf
...
xmlDocPtr write_config(char *confname, char *keyname, char *value, int pid, int mode)
{
	...
	remove("/etc/munin.conf");
	FILE *conffile = fopen("/etc/munin.conf", "w+");
	if (!conffile) {
		fprintf(stderr,"error opening conffile to write: %m");
	}
...
int main(int argc, char **argv)
{
	...
	if (started == 1 && daemon == 0) {
		...
		self = malloc(sizeof(struct munin));
...
w_status change_conf(struct munin *self, int fanotify_fd)
{
	...
	doc = xmlParseFile("/etc/munin.conf");
	if (doc == NULL ) {
 ...
 *
 * @munin: pointer to the own data structure
 */
void *output(void *munin)
{
	GHashTableIter iter;
	gpointer key, value;
	int res;
	struct munin *self = (struct munin *)munin;
	self->completed_out = 0;
